/**
 * Segment Trigger Service Tests
 *
 * Tests for the Segment trigger service that manages webhook-based triggers.
 */
import { Test, TestingModule } from '@nestjs/testing';
import { SegmentTriggerService } from '../segment-trigger.service';
...
describe('SegmentTriggerService', () => {
  let service: SegmentTriggerService;
  let platformService: jest.Mocked<PlatformService>;
  let workflowService: jest.Mocked<WorkflowService>;

  const mockWorkflowId = 'workflow-123';
  const mockCredentialId = 'credential-456';
  const mockWriteKey = 'test-write-key-123';
  const mockWorkspaceId = 'test-workspace-id';
  const mockWebhookUrl = 'http://localhost:5005/webhooks/segment/workflow-123';
  ...
  beforeEach(async () => {
    ...
    service = module.get<SegmentTriggerService>(SegmentTriggerService);
  });
  ...
  describe('getTriggerType', () => {
    it('should return SEGMENT trigger type', () => {
      const triggerType = service.getTriggerType();
      expect(triggerType).toBe(TriggerType.SEGMENT);
    });
  });
  ...
  describe('activate', () => {
    ...
    it('should activate trigger successfully', async () => {
      ...
      expect(result.data?.webhookUrl).toContain('/webhooks/segment/');
      expect(result.data?.triggerId).toBe('audience_entered');
    ...
    it('should fail when writeKey is missing', async () => {
      platformService.query.mockResolvedValueOnce({
        rows: [{ credentials: { workspace_id: mockWorkspaceId } }],
        rowCount: 1,
        command: 'SELECT',
        fields: [],
      } as any);
      ...
      expect(result.success).toBe(false);
      expect(result.message).toContain('Invalid Segment credentials');
    });
    ...
    it('should return trigger event types in activation result', async () => {
      ...
      expect(result.success).toBe(true);
      expect(result.data?.triggerTypes).toContain('segment:audience_entered');
    });

    it('should default to audience_entered trigger when not specified', async () => {
      platformService.query.mockResolvedValueOnce({
        rows: [
          {
            credentials: {
  ...
  describe('getStatus', () => {
    it('should return active status when webhook is configured', async () => {
      ...
      expect(result.type).toBe(TriggerType.SEGMENT);
      expect(result.message).toContain('configured');
      expect(result.metadata?.webhookUrl).toBe(mockWebhookUrl);
      expect(result.metadata?.workspaceId).toBe(mockWorkspaceId);
    });
