 ...
 * Similar to n8n's schedule helper functions
 */
...
export function scheduleToCron(config: ScheduleConfig): string {
  const { mode, interval, hour = 0, minute = 0, dayOfWeek = [], dayOfMonth = 1, cronExpression } = config;
  ...
  switch (mode) {
    ...
    case "days":
      ...
      if (interval === 1) {
        ...
        // Every X days (note: cron doesn't natively support "every X days", so we use specific days)
        // For simplicity, we'll use daily and let the application logic handle the interval
...
/**
 * Parse cron expression back to friendly format (best effort)
 * This is useful when loading existing schedules
 */
export function cronToSchedule(cron: string): Partial<ScheduleConfig> {
  const parts = cron.trim().split(/\s+/);
  if (parts.length !== 5) {
    // Invalid cron, return as custom
    return {
      mode: "cron",
      cronExpression: cron,
    };
  ...
  // Every X hours at minute Y: Y */X * * *
  if (hourPart.startsWith("*/") && dayOfMonthPart === "*" && monthPart === "*" && dayOfWeekPart === "*") {
  ...
  // Daily: M H * * *
  if (!minutePart.includes("*") && !hourPart.includes("*") && dayOfMonthPart === "*" && monthPart === "*" && dayOfWeekPart === "*") {
    return {
      mode: "days",
      interval: 1,
  ...
  // Weekly: M H * * D
  if (!minutePart.includes("*") && !hourPart.includes("*") && dayOfMonthPart === "*" && monthPart === "*" && dayOfWeekPart !== "*") {
    const days = dayOfWeekPart.split(",").map((d) => parseInt(d));
    return {
      mode: "weeks",
      interval: 1,
      hour: parseInt(hourPart),
  ...
  // Monthly: M H D * *
  if (!minutePart.includes("*") && !hourPart.includes("*") && !dayOfMonthPart.includes("*") && monthPart === "*" && dayOfWeekPart === "*") {
    return {
      mode: "months",
      interval: 1,
      hour: parseInt(hourPart),
      minute: parseInt(minutePart),
      dayOfMonth: parseInt(dayOfMonthPart),
    };
  }

  // If we can't parse it, return as custom cron
  return {
    mode: "cron",
    cronExpression: cron,
  };
